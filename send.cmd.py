...
print("Please look at NetBox Structure if unsure and this tool is primarily intended for show commands and runs outside of Enable mode.")
print("There is another script in the making that allows you to send multiple rows, for example an ACL.\n")
...
print("")
print_title("Now Enter command you want to send (Dont worry, it wont send it until later when you confirm )")
command = input ("Command: ")


#Defining the actual push command from Netmiko

def baseconfig(push):
    #Send config textfile to targets push.run(task=netmiko_send_config, config_file= "config_textfile")
    push.run(task=netmiko_send_command, command_string = command)
...
print_title("You have now added filters, now it's time to apply them \n[P] Platform\n[PR] Platform,Role\n[PRS] Platform,Role,Site ")
